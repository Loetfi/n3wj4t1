...
class AZAppCRUD extends AZ {
	...
	// $this->ci->db->join($data, $data.".".$join_x." = "$other".".$join_x, $type);
	...
	public function get_table() {
		...
		if($cfilter != ''){
			foreach($cfilter as $pcf_k => $pcf_v){
				$pcf_k = "product.name"; //$this->ci->encryption->decode($pcf_k);
				if(strlen($pcf_v) > 0){
					$this->ci->db->like("product.name", $pcf_v);
				}
			}
		}	
		...
		if($cfilter != ''){
			foreach($cfilter as $pcf_k => $pcf_v){
				$pcf_k = 'product.name'; //$this->ci->encrypt->decode($pcf_k);
				if(strlen($pcf_v) > 0){
		...
		foreach ($ambil->result_array() as $value) {
			...
			$btn_ = "";
			if ($this->edit) {
				$btn_ .= '<button type="button" class="btn btn-info btn-xs btn-select-product" data-dismiss="modal" data-idproduct="'.$this->id.'" data-barcode="AZT-001111" data-name="123" data-price="12311" data-stock="1603"><span class="glyphicon glyphicon-arrow-down"></span> Pilih</button></div>';

				// $btn_ .= '<button class="btn btn-default btn-xs btn-edit-'.$this->id.'" data_id= "'.$value['id'.$table].'"><span class="glyphicon glyphicon-pencil"></span> Edit</button>';
			}
			if ($this->delete) {
				// $btn_ .= '<button class="btn btn-danger btn-xs btn-delete-'.$this->id.'" data_id= "'.$value['id'.$table].'"><span class="glyphicon glyphicon-remove"></span> Delete</button>';
			}
